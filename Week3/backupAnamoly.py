...
# -----------------------------
# Populate Dummy Backup Metrics
# -----------------------------
def populate_dummy_metrics(num_records=5):
    """
    Insert dummy normal backup durations into the metrics table.
    """
    with psycopg2.connect(**conn_params) as conn:
        with conn.cursor() as cur:
            for _ in range(num_records):
                cur.execute("INSERT INTO backup_metrics (duration, status) VALUES (%s, %s);", (0.1, 'SUCCESS'))
            conn.commit()
    logging.info(f"Inserted {num_records} dummy normal backup records.")
...
def backup_data():
    ...
    try:
        # Artificial delay to simulate a long backup process (anomaly scenario)
...
def restore_data():
    try:
        ...
        with psycopg2.connect(**conn_params) as conn:
            with conn.cursor() as cur:
                # Optionally remove existing data in the source table before restoration
...
if __name__ == "__main__":
    # Ensure the source and metrics tables exist.
    create_source_table_if_not_exists()
    create_or_recreate_metrics_table()
    
    # Populate the metrics table with dummy normal records.
    populate_dummy_metrics(num_records=5)

    # Perform the backup process (this one will be delayed to simulate an anomaly).
